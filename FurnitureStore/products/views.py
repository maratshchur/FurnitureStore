...
from portal.models import Article, News, Partner, Worker
...
from .forms import OrderForm, PaymentForm, PickupPointForm, ProductForm, ProductTypeForm, PromoCodeForm, WorkerForm
...
def home(request):
    ...
    products = Product.objects.all()  # Передаем все товары
...
def employees_page(request):
    return render(request, 'products/employee_manage.html') # Create this template file

def employees_manage(request):
    # Параметры сортировки и фильтрации
    sort_column = request.GET.get('sort', 'last_name')
    sort_direction = request.GET.get('direction', 'asc')
    filter_text = request.GET.get('filter', '')

    # Список допустимых полей для сортировки
    valid_sort_columns = ['email', 'first_name', 'id', 'job_description', 'last_name', 'phone_number', 'photo']
    
    # Проверка на допустимость поля сортировки
    if sort_column not in valid_sort_columns:
        sort_column = 'last_name'  # Или можно установить в значение по умолчанию

    # Фильтрация по имени или фамилии
    employees =Worker.objects.filter(
        last_name__icontains=filter_text
    ) |  Worker.objects.filter(
        first_name__icontains=filter_text
    )
    ...
    if sort_direction == 'asc':
        ...
        employees = employees.order_by(f'-{sort_column}')
    ...
    employees_data = [{
        ...
        'photo': worker.photo.url if worker.photo else '',
...
def employee_add(request):
    ...
    if form.is_valid():
        # Если форма валидна, сохраняем нового сотрудника
        worker = form.save()
        
        # Возвращаем успешный ответ с данными нового сотрудника
        return JsonResponse({
            'status': 'success',
...
def js_var1(request):
    return render(request, 'products/js_var1.html')

def js_var2(request):
    return render(request, 'products/js_var2.html')
...
def func_plots(request):
    return render(request, 'portal/func_plots.html')
