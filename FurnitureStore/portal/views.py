...
from .models import Article, Company, CompanyInfo, News, Term,  Vacancy, Review, Worker
...
def about(request):
    ...
    return render(request, 'portal/about.html', {'company': company})
...
def employee_list(request):
    ...
    return render(request, 'portal/workers.html', {'employees': employees})
...
def add_review(request):
    if request.method == 'POST':
        ...
        review = Review(user=request.user.customer, rating=rating, text=text)
        review.save()
        
        return redirect('reviews')
    
    return render(request, 'portal/add_review.html')
